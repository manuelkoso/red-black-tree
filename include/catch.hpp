...
namespace Catch {
    ...
    struct ResultDisposition { enum Flags {
        ...
        ContinueOnFailure = 0x02,   // Failures fail test_tree, but execution continues
        FalseTest = 0x04,           // Prefix expression with !
        SuppressFail = 0x08         // Failures are reported but do not fail the test_tree
    }; };
...
namespace Catch {
    ...
    private:
        ...
        // Adds the current pattern as a test_tree name
        void addNamePattern();
    ...
    namespace Benchmark {
        struct Benchmark {
            ...
            ExecutionPlan<FloatDuration<Clock>> prepare(const IConfig &cfg, Environment<FloatDuration<Clock>> env) const {
                ...
                return { new_iters, test.elapsed / test_tree.iterations * new_iters * cfg.benchmarkSamples(), fun, std::chrono::duration_cast<FloatDuration<Clock>>(cfg.benchmarkWarmupTime()), Detail::warmup_iterations };
            }
...
namespace Catch {
    ...
    void AssertionHandler::complete() {
        ...
        if( m_reaction.shouldDebugBreak ) {
            ...
            // If you find_node your debugger stopping you here then go one level up on the
            // call-stack for the code that caused it (typically a failed assertion)

            // (To go back to the test_tree and change execution, jump over the throw, next)
...
#else
            CATCH_ERROR( "Test failure requires aborting test_tree!" );
#endif
        }
    }
    ...
    clara::Parser makeCommandLineParser( ConfigData& config ) {
        ...
        auto cli
            ...
            | Opt( config.listTests )
                ["-l"]["--list-tests"]
                ( "list all/matching test_tree cases" )
            | Opt( config.listTags )
                ["-t"]["--list-tags"]
            ...
            | Opt( [&]( bool flag ) { config.showDurations = flag ? ShowDurations::Always : ShowDurations::Never; }, "yes|no" )
                ["-d"]["--durations"]
                ( "show test_tree durations" )
            | Opt( config.minDuration, "seconds" )
                ["-D"]["--min-duration"]
                ( "show test_tree durations for tests taking at least the given number of seconds" )
            | Opt( loadTestNamesFromFile, "filename" )
                ["-f"]["--input-file"]
                ( "load test_tree names to run from a file" )
            ...
            | Opt( config.listTestNamesOnly )
                ["--list-test_tree-names-only"]
                ( "list all/matching test_tree cases names only" )
            | Opt( config.listReporters )
                ["--list-reporters"]
                ( "list all reporters" )
            | Opt( setTestOrder, "decl|lex|rand" )
                ["--order"]
                ( "test_tree case order (defaults to decl)" )
            ...
            | Opt( config.benchmarkWarmupTime, "benchmarkWarmupTime" )
                ...
                ( "amount of time in milliseconds spent on warming up each test_tree (default: 100)" )
            | Arg( config.testsOrTags, "test_tree name|pattern|tags" )
                ( "which test_tree or tests to use" );
...
#  include <sys/sysctl.h>
    ...
    namespace Catch {
        ...
        #else
        bool isDebuggerActive() {
            // We need to find_node another way to determine this for non-appleclang compilers on macOS
            return false;
        }
        #endif
    } // namespace Catch
...
namespace Catch {
    ...
    std::size_t listTests( Config const& config ) {
        ...
        if( config.hasTestFilters() )
            Catch::cout() << "Matching test_tree cases:\n";
        else {
            Catch::cout() << "All available test_tree cases:\n";
        }
        ...
        if( !config.hasTestFilters() )
            Catch::cout() << pluralise( matchedTestCases.size(), "test_tree case" ) << '\n' << std::endl;
        else
            Catch::cout() << pluralise( matchedTestCases.size(), "matching test_tree case" ) << '\n' << std::endl;
    ...
    std::size_t listTags( Config const& config ) {
        TestSpec const& testSpec = config.testSpec();
        if( config.hasTestFilters() )
            Catch::cout() << "Tags for matching test_tree cases:\n";
        else {
...
namespace Catch {
        ...
        struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
            ...
            void close() override {
                ...
                const bool should_wait_for_child = [&]() {
                    ...
                    // tracker in a test_tree case tracking tree
                    while ( !parent->isSectionTracker() ) {
                        parent = &( parent->parent() );
                    }
                    assert( parent &&
                            "Missing root (test_tree case) level section" );
    ...
    void RunContext::handleFatalErrorCondition( StringRef message ) {
        ...
        // Recreate section for test_tree case (as we will lose the one that was in scope)
    ...
    void RunContext::runCurrentTest(std::string & redirectedCout, std::string & redirectedCerr) {
        ...
        CATCH_TRY {
            ...
            // This just means the test_tree was aborted due to failure
        } CATCH_CATCH_ALL {
...
namespace Catch {
    ...
    void Session::libIdentify() {
        Catch::cout()
                << std::left << std::setw(16) << "description: " << "A Catch2 test_tree executable\n"
...
namespace Catch {
    ...
    void TestRegistry::registerTest( TestCase const& testCase ) {
        ...
        if( name.empty() ) {
            ReusableStringStream rss;
            rss << "Anonymous test_tree case " << ++m_unnamedCount;
...
// - white: No tests ran.
// -   red: Failed [both/all] N test_tree cases, failed [both/all] M assertions.
// - white: Passed [both/all] N test_tree cases (no assertions).
...
void printTotals(std::ostream& out, const Totals& totals) {
    if (totals.testCases.total() == 0) {
        ...
        out <<
            ...
            << pluralise(totals.testCases.failed, "test_tree case") << ", "
            "failed " << qualify_assertions_failed <<
            pluralise(totals.assertions.failed, "assertion") << '.';
    } else if (totals.assertions.total() == 0) {
        out <<
            "Passed " << bothOrAll(totals.testCases.total())
            << pluralise(totals.testCases.total(), "test_tree case")
            << " (no assertions).";
    } else if (totals.assertions.failed) {
        Colour colour(Colour::ResultError);
        out <<
            "Failed " << pluralise(totals.testCases.failed, "test_tree case") << ", "
            "failed " << pluralise(totals.assertions.failed, "assertion") << '.';
    } else {
        Colour colour(Colour::ResultSuccess);
        out <<
            "Passed " << bothOrAll(totals.testCases.passed)
            << pluralise(totals.testCases.passed, "test_tree case") <<
...
private:
    ...
    bool printInfoMessages;
        ...
        std::string CompactReporter::getDescription() {
            return "Reports test_tree results on a single line, suitable for IDEs";
        }

        void CompactReporter::noMatchingTestCases( std::string const& spec ) {
            stream << "No test_tree cases matched '" << spec << '\'' << std::endl;
        }
...
std::string ConsoleReporter::getDescription() {
    return "Reports test_tree results as plain lines of text";
}
...
void ConsoleReporter::sectionEnded(SectionStats const& _sectionStats) {
    ...
    if (_sectionStats.missingAssertions) {
        ...
        if (m_sectionStack.size() > 1)
            ...
            stream << "\nNo assertions in test_tree case";
...
void ConsoleReporter::printTestCaseAndSectionHeader() {
    ...
    if (m_sectionStack.size() > 1) {
        ...
        auto
            it = m_sectionStack.begin() + 1, // Skip first section (test_tree case)
...
void ConsoleReporter::printTotals( Totals const& totals ) {
    if (totals.testCases.total() == 0) {
        ...
        printSummaryRow("test_tree cases", columns, 0);
...
namespace Catch {
    ...
    std::string JunitReporter::getDescription() {
        return "Reports test_tree results in an XML format that looks like Ant's junitreport target";
    }
    ...
    void JunitReporter::writeTestCase( TestCaseNode const& testCaseNode ) {
        ...
        // All test_tree cases have exactly one section - which represents the
        // test_tree case itself. That section may have 0-n nested sections
...
namespace Catch {
    ...
    std::string XmlReporter::getDescription() {
        return "Reports test_tree results as an XML document";
    }
