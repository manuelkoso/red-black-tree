...
void RBTree<T, CMP>::insert_fixup(Node *new_node) {
    while (new_node && new_node->parentExist() && new_node->parent->checkColor(node_color::red)) {
        if (new_node->parent->parentExist() && new_node->parent == new_node->parent->parent->left.get()) {
            Node *y = new_node->parent->parent->right.get();
            if (y && y->checkColor(node_color::red)) {
                new_node->parent->color = node_color::black;
                y->color = node_color::black;
                new_node->parent->parent->color = node_color::red;
                new_node = new_node->parent->parent;
            } else {
                if (new_node->isRightChild()) {
...
void RBTree<T, CMP>::delete_fixup(RBTree::Node *x, Node *xp) {
    while (x != root.get() && (!x || x->checkColor(node_color::black))) {
        if (xp->left.get() == x) {
            Node *w = xp->right.get();
            if (w && w->checkColor(node_color::red)) {
                w->color = node_color::black;
                xp->color = node_color::red;
                left_rotate(get_uniq_pointer(xp));
                w = xp->right.get();
            }
            if (w && (!w->left || w->left->checkColor(node_color::black)) &&
                (!w->right || w->right->checkColor(node_color::black))) {
                w->color = node_color::red;
                x = xp;
                xp = x->parent;
            } else if (w) {
                if (!w->right || w->right->checkColor(node_color::black)) {
            ...
            if (w && (!w->right || w->right->checkColor(node_color::black)) &&
                (!w->left || w->left->checkColor(node_color::black))) {
                w->color = node_color::red;
                x = xp;
                xp = x->parent;
            } else if (w) {
                if (!w->left || w->left->checkColor(node_color::black)) {
                    if (w->right) w->right->color = node_color::black;
                    w->color = node_color::red;
